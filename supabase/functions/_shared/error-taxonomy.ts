/**
 * Taxonomia de Erros Padronizada
 * ==============================
 * 
 * Este arquivo define códigos de erro padronizados para todo o sistema,
 * facilitando o debug rápido através de filtros no agent_logs.
 * 
 * Categorias:
 * - LLM_*: Erros relacionados à IA/LLM
 * - SYNTAX_*: Erros de sintaxe de código
 * - IMPORT_*: Erros de imports/dependências
 * - STUB_*: Erros de geração de stubs
 * - WC_*: Erros do WebContainer
 * - STREAM_*: Erros de streaming
 * - VALIDATION_*: Erros de validação geral
 * - NETWORK_*: Erros de rede/API
 */
...
/**
 * Filtra erros por categoria
 */
export function filterErrorsByCategory(errors: AppError[], category: ErrorCategory): AppError[] {
    return errors.filter(e => e.category === category);
}

/**
 * Filtra erros por severidade mínima
 */
export function filterErrorsBySeverity(
    ...
    return errors.filter(e => severityOrder.indexOf(e.severity) >= minIndex);
}
